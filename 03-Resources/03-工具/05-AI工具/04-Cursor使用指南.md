...
为了确保 Cursor 更准确、可靠地执行你的指令，尤其是在处理复杂或关键任务时，建议采用以下交互策略：

1.  **复述需求 (Ask & Confirm):** 在开始执行复杂任务前，要求 Cursor 复述一遍你的需求，确保理解一致。这有助于避免方向性错误。
2.  **审阅数据 (Review First):** 在处理数据或执行文件操作（如修改、移动、删除）前，要求 Cursor 先审阅相关数据或文件内容（可以提示其使用 `mcp_sequential-thinking_sequentialthinking` 进行分析），了解当前状态，避免基于错误假设的操作。
3.  **交叉验证与记录 (Verify & Log):** 对于多步骤任务，要求 Cursor 对关键步骤的结果进行交叉验证（例如，检查文件是否真的被移动、代码是否按预期修改、命令是否成功执行），并将每一步的操作和决策记录在案（例如，通过日志文件、在相关笔记中添加条目或在对话中明确说明）。
4.  **规划先行 (Plan Before Action):** 在执行整理、重构、规划或其他涉及多项改动的任务前，要求 Cursor 先根据你的需求提出一个清晰的执行计划或整理方案。待你确认计划可行后，再让其开始执行。 
